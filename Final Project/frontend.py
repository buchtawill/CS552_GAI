...
def get_game_art(game_setup:dict, locs:dict, npcs:dict):
    ...
    for loc_name in locs:
        ...
        print(f"INFO [get_game_art()] Requesting textures for {loc_name}")
    ...
    prompt = f"A very lightly-detailed background floor tile with the theme {game_setup['theme']}. The color palette is light."
    floor_tile_path, _ = get_pixel_art(prompt, size=(TILE_SIZE, TILE_SIZE), style="texture", b_tile=True, rm_bg=False)
    
    # Generate wall tile for world boundary
    write_to_log(f"INFO [frontend.py::get_game_art()] Getting wall tile")
    prompt = f"A Wall tile according to the theme {game_setup['theme']}. This is a world boundary for the game, and should be dense."
...
if __name__ == "__main__":
    ...
    with open('resources/art_assets_cyberpunk.pkl', 'wb') as f:
        pickle.dump(art_paths, f)
    with open('resources/game_premise_cyberpunk.pkl', 'wb') as f:
        pickle.dump(premise, f)
        
    # with open('resources/art_assets_cyberpunk.pkl', 'rb') as f:
    #     art_paths = pickle.load(f)
    #     art_assets = load_assets_from_paths(art_paths)
    # with open('resources/game_premise_cyberpunk.pkl', 'rb') as f:
    #     premise = pickle.load(f)

    # --- Player Setup (World Coordinates) ---
    player_world_pos = pygame.Vector2(base_res[0]//2, base_res[1]//2)
    ...
    while running:
        ...
        # Render tile_colors[0] everywhere
        ...
        # Draw player x and y
        position_text = f"X: {int(player_world_pos.x)}  Y: {int(player_world_pos.y)}"
